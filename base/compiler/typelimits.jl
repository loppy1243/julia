...
function issimplertype(@nospecialize(typea), @nospecialize(typeb))
    ...
    if typea isa PartialStruct
        ...
        for i = 1:length(typea.fields)
            ...
            is_lattice_equal(ai, bi) && continue
            # is is not enough for ai to be simpler than bi, it must exactly equal
            # (as an invariant struct field, by contrast to type_more_complex above which handles covariant tuples)
            return false
        end
